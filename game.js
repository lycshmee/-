...
//通过定时器(setTimeout和setlterval)间隔来改变元素样式实现动画效果时，就会出现上面所说的掉帧造成图片闪烁感有点高。所以强烈推荐使用requestAnimationFrame优化Web动画
...
function Game(id, params){
    ...
    Stage.prototype.createItem = function(options){
        ...
        //动态属性
        if(item.location){
            Object.assign(item,item.location.coordtoposition(item.coord.x,item.coord.y));
        }
        //关系绑定
        item._stage = this;
